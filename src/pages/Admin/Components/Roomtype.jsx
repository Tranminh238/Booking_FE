...
import {
    Plus, Search, Edit2, Trash2, RefreshCw, X, AlertTriangle, Tag, Loader2
} from "lucide-react";
import { message } from "antd";

const API_BASE = "http://localhost:8889/api/room-types";
...
export default function Roomtype() {
    ...
    // Form states
    const [currentRoomType, setCurrentRoomType] = useState(null); // null if creating, roomType object if editing
    ...
    const handleOpenDelete = (roomType) => {
        setCurrentRoomType(roomType);
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.name.trim()) {
            setValidationError("Tên loại phòng không được để trống.");
            return;
        }
        setValidationError("");
        setSubmitting(true);
        ...
        try {
            const isEdit = !!currentRoomType;
            const url = isEdit
                ? `${API_BASE}/update/${currentRoomType.id}`
                : `${API_BASE}/create`;
            ...
            if (!res.ok) {
                // Try to check if backend sent a message (could be runtime error like duplicated name)
            ...
            message.success(isEdit ? "Cập nhật loại phòng thành công!" : "Tạo loại phòng thành công!");
    ...
    const handleDeleteConfirm = async () => {
        ...
        setSubmitting(true);
        try {
            const res = await fetch(`${API_BASE}/delete/${currentRoomType.id}`, {
                method: "DELETE",
            });
            const text = await res.text();
            if (!res.ok) {
            ...
            console.error(err);
            message.error(err.message || "Không thể xóa loại phòng.");
        } finally {
            setSubmitting(false);
        }
    };
                ...
                <div className="max-w-7xl mx-auto space-y-6">
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm px-6 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h1 className="text-xl font-bold text-slate-900 leading-tight">Quản lý loại phòng</h1>
                                <p className="text-xs text-slate-400 mt-0.5">Tổng số loại phòng hiện tại: {roomTypes.length}</p>
                            </div>
                        </div>
                    ...
                    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                        {loading ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-24 text-center">
                                ...
                                <h3 className="text-slate-700 font-semibold text-lg">Không tìm thấy loại phòng</h3>
                                <p className="text-slate-400 text-sm mt-1 max-w-xs mx-auto">
                                    {search ? "Vui lòng kiểm tra lại từ khóa tìm kiếm của bạn hoặc thử từ khóa khác." : "Hệ thống chưa có dữ liệu loại phòng nào."}
                                </p>
                            </div>
                        ) : (
                                ...
                                <table className="w-full text-left">
                                    ...
                                    <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
                                        {filteredRoomTypes.map((rt) => (
                                            <tr key={rt.id} className="hover:bg-slate-50/50 transition-colors group">
                                                <td className="px-6 py-4 font-mono text-slate-400 font-medium">#{rt.id}</td>
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden z-10 border border-slate-100 animate-fade-in">
                        ...
                        <div className="px-6 py-4 bg-slate-50 border-b border-slate-100 flex items-center justify-between">
                            <h2 className="text-base font-bold text-slate-900">
                                {currentRoomType ? "Cập nhật loại phòng" : "Thêm loại phòng mới"}
                            </h2>
                            <button
                                disabled={submitting}
                            ...
                            <div className="p-6 space-y-4">
                                <div className="space-y-3">
                                    ...
                                    <input
                                        ...
                                        id="name"
                                        autoFocus
                                        placeholder="Ví dụ: Deluxe Suite, Standard Room..."
                                        disabled={submitting}
                                        value={formData.name}
                                        onChange={(e) => {
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden z-10 border border-slate-100 animate-fade-in">
                        <div className="p-6 text-center">
                            ...
                            <h3 className="text-slate-900 font-bold text-lg">Xác nhận xóa loại phòng?</h3>
                            <p className="text-slate-400 text-sm mt-2">
                                Bạn có chắc chắn muốn xóa loại phòng <span className="font-semibold text-slate-700">"{currentRoomType?.name}"</span>? Hành động này không thể được hoàn tác.
                            </p>
                        </div>
